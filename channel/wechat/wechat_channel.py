...
class WechatChannel(Channel):
    ...
    def startup(self):
        ...
        self.pass_ticket = itchat.instance.loginInfo["pass_ticket"]
        # print("nnnnn",itchat.instance.loginInfo)
    ...
    def handle_group(self, msg):
        ...
        # print("handle_groupvhandle_grouphandle_group", msg)
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            ...
            group_chat_zidingyi = check_prefix_zidingyi(content, conf().get('group_chat_zidingyi')) # 指定的自定义的回复
            if img_match_prefix:
                content = content.replace(img_match_prefix, '', 1).strip()
                context.type = ContextType.IMAGE_CREATE
            elif group_chat_zidingyi:
                context.type = ContextType.ZIDINGYI
            else:
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        # print(context.type,"afdsfasdfasdfadsfasfdasdf", context.content, context)
        if not e_context.is_pass():
            logger.debug('[WX] ready to handle context: type={}, content={}'.format(context.type, context.content))
            if context.type == ContextType.TEXT or context.type == ContextType.IMAGE_CREATE:  # 文字和图片消息
                reply = super().build_reply_content(context.content, context)
            elif context.type == ContextType.ZIDINGYI:  # 自定义的消息发送
                print("走到这里自定义回复了",self.pass_ticket)
                reply = super().build_reply_content_zidingyi(context.content, context,pass_ticket=self.pass_ticket)
...
def check_prefix(content, prefix_list):
    ...
    return None
# 查询自定义
def check_prefix_zidingyi(content, prefix_list):
    for prefix in prefix_list:
        if content.startswith(prefix):
            return prefix
    return None
